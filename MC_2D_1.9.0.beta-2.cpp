...
- 修复了如下 BUG：
	...
	- 开始菜单 BUG
- 优化了码风
- 增加了移动摁键（将大写W、S、A、D加入摁键中）（避免像Corange一样的手残党玩家不小心摁到大写锁定后被定身）
- 将原本需要摁住Shift才能产生效果的摁键改为了不用摁住Shift的键（摁键位置不变，原来那些需要摁住Shift的键也没删） 
...
void main_menu() { //主菜单
	...
	cout << "          " << "按w，s来选择吧！" << endl;
	cout << "          " << "1.新游戏";
	if (main_menu_point == 0) { cout << "√";}
	cout << endl;
	cout << "          " << "2.我要读档！！！";
	if (main_menu_point == 1) {	cout << "√";}
	cout << endl;
	cout << "          " << "空格确定" << endl;
}
void ad() { //广子
